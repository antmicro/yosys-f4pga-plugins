...
void UhdmAst::process_module()
{
    ...
    if (!is_module_instance) {
        if (shared.top_nodes.find(type) != shared.top_nodes.end()) {
            ...
            visit_one_to_many({vpiModule, vpiNet, vpiArrayNet, vpiGenScopeArray, vpiProcess}, obj_h,
                              [&](AST::AstNode *node) {
                                  if (node) {
        ...
        visit_one_to_many({vpiInterface, vpiModule, vpiPort, vpiGenScopeArray, vpiContAssign, vpiParameter, vpiParamAssign}, obj_h, [&](AST::AstNode *node) {
            if (node) {
                add_or_replace_child(module_node, node);
            }
        });
