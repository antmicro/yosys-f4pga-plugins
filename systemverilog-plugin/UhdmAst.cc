...
{
    if (!to->attributes.count(attr_id::packed_ranges))
        to->attributes[attr_id::packed_ranges] = AST::AstNode::mkconst_int(1, false, 1);
    if (!to->attributes.count(attr_id::unpacked_ranges))
        to->attributes[attr_id::unpacked_ranges] = AST::AstNode::mkconst_int(1, false, 1);
    if (from->attributes.count(attr_id::packed_ranges)) {
        for (auto r : from->attributes[attr_id::packed_ranges]->children) {
            to->attributes[attr_id::packed_ranges]->children.push_back(r->clone());
        }
    }
    if (from->attributes.count(attr_id::unpacked_ranges)) {
        for (auto r : from->attributes[attr_id::unpacked_ranges]->children) {
            to->attributes[attr_id::unpacked_ranges]->children.push_back(r->clone());
        }
    }
}
...
static void add_multirange_wire(AST::AstNode *node, std::vector<AST::AstNode *> packed_ranges, std::vector<AST::AstNode *> unpacked_ranges,
                                bool reverse = true)
{
    node->attributes[attr_id::packed_ranges] = AST::AstNode::mkconst_int(1, false, 1);
    if (!packed_ranges.empty()) {
        if (reverse)
            std::reverse(packed_ranges.begin(), packed_ranges.end());
        node->attributes[attr_id::packed_ranges]->children.insert(node->attributes[attr_id::packed_ranges]->children.end(), packed_ranges.begin(),
                                                                  packed_ranges.end());
    }

    node->attributes[attr_id::unpacked_ranges] = AST::AstNode::mkconst_int(1, false, 1);
    if (!unpacked_ranges.empty()) {
        if (reverse)
            std::reverse(unpacked_ranges.begin(), unpacked_ranges.end());
        node->attributes[attr_id::unpacked_ranges]->children.insert(node->attributes[attr_id::unpacked_ranges]->children.end(),
...
static void resolve_wiretype(AST::AstNode *wire_node)
{
    ...
    if (wire_node->attributes.count(attr_id::packed_ranges)) {
        for (auto r : wire_node->attributes[attr_id::packed_ranges]->children) {
            packed_ranges.push_back(r->clone());
        }
    }
    if (wire_node->attributes.count(attr_id::unpacked_ranges)) {
        for (auto r : wire_node->attributes[attr_id::unpacked_ranges]->children) {
    ...
    if ((wire_node->children[0]->type == AST::AST_RANGE || (wire_node->children.size() > 1 && wire_node->children[1]->type == AST::AST_RANGE)) &&
        ...
        if (wiretype_ast && !wiretype_ast->children.empty() && wiretype_ast->children[0]->attributes.count(attr_id::packed_ranges) &&
            wiretype_ast->children[0]->attributes.count(attr_id::unpacked_ranges)) {
            for (auto r : wiretype_ast->children[0]->attributes[attr_id::packed_ranges]->children) {
                packed_ranges.push_back(r->clone());
            }
            for (auto r : wiretype_ast->children[0]->attributes[attr_id::unpacked_ranges]->children) {
                unpacked_ranges.push_back(r->clone());
            }
        } else {
        ...
        wire_node->attributes[attr_id::packed_ranges] = AST::AstNode::mkconst_int(1, false, 1);
        if (!packed_ranges.empty()) {
            std::reverse(packed_ranges.begin(), packed_ranges.end());
            wire_node->attributes[attr_id::packed_ranges]->children.insert(wire_node->attributes[attr_id::packed_ranges]->children.end(),
                                                                           packed_ranges.begin(), packed_ranges.end());
        }

        wire_node->attributes[attr_id::unpacked_ranges] = AST::AstNode::mkconst_int(1, false, 1);
        if (!unpacked_ranges.empty()) {
            wire_node->attributes[attr_id::unpacked_ranges]->children.insert(wire_node->attributes[attr_id::unpacked_ranges]->children.end(),
                                                                             unpacked_ranges.begin(), unpacked_ranges.end());
        }
    }
}
...
{
    wire_node->attributes[attr_id::force_convert] = AST::AstNode::mkconst_int(val, true);
}
    ...
    visitEachDescendant(module_node, [&](AST::AstNode *node) {
        ...
        if (node->type == AST::AST_WIRE) {
            const std::vector<AST::AstNode *> packed_ranges =
              node->attributes.count(attr_id::packed_ranges) ? node->attributes[attr_id::packed_ranges]->children : std::vector<AST::AstNode *>();
            const std::vector<AST::AstNode *> unpacked_ranges =
              node->attributes.count(attr_id::unpacked_ranges) ? node->attributes[attr_id::unpacked_ranges]->children : std::vector<AST::AstNode *>();
        ...
        if (node->type == AST::AST_IDENTIFIER && memories.count(node->str)) {
            if (!memories[node->str]->attributes.count(attr_id::force_convert)) {
...
static void convert_packed_unpacked_range(AST::AstNode *wire_node)
{
    ...
    const std::vector<AST::AstNode *> packed_ranges =
      wire_node->attributes.count(attr_id::packed_ranges) ? wire_node->attributes[attr_id::packed_ranges]->children : std::vector<AST::AstNode *>();
    const std::vector<AST::AstNode *> unpacked_ranges = wire_node->attributes.count(attr_id::unpacked_ranges)
                                                          ? wire_node->attributes[attr_id::unpacked_ranges]->children
    ...
    const bool convert_node = [&]() {
        ...
        if ((wire_node->is_input || wire_node->is_output) && (packed_ranges.size() > 0 || unpacked_ranges.size() > 0))
            return true;
        if (wire_node->attributes.count(attr_id::force_convert) && wire_node->attributes[attr_id::force_convert]->integer == 1)
            return true;
        return false;
    }();
    if (convert_node) {
        ...
        if (wire_node->multirange_dimensions.empty()) {
            size_t packed_size = add_multirange_attribute(wire_node, packed_ranges);
            ...
            if (packed_ranges.size() == 1 && unpacked_ranges.empty()) {
                ...
                size_t size = packed_size * unpacked_size;
                log_assert(size >= 1);
...
static AST::AstNode *convert_dot(AST::AstNode *wire_node, AST::AstNode *node, AST::AstNode *dot)
{
    ...
    if (node->children[0]->type == AST::AST_RANGE) {
        ...
        if (!wire_node->attributes[attr_id::unpacked_ranges]->children.empty() &&
            wire_node->attributes[attr_id::unpacked_ranges]->children.back()->range_left == range) {
...
static AST::AstNode *make_packed_struct_local(AST::AstNode *template_node, std::string &name)
{
    ...
    wnode->attributes[attr_id::packed_ranges]->children.insert(wnode->attributes[attr_id::packed_ranges]->children.begin(), range);
...
static void simplify(AST::AstNode *current_node, AST::AstNode *parent_node)
{
    ...
    case AST::AST_FUNCTION:
        ...
        break;
    case AST::AST_WIRE:
        current_node->attributes[attr_id::is_simplified_wire] = AST::AstNode::mkconst_int(1, true);
        [[fallthrough]];
    ...
    case AST::AST_IDENTIFIER:
        if (!current_node->children.empty() && !current_node->basic_prep) {
            ...
            if (!wire_node->attributes.count(attr_id::is_simplified_wire)) {
                simplify(wire_node, nullptr);
            }
            const std::vector<AST::AstNode *> packed_ranges = wire_node->attributes.count(attr_id::packed_ranges)
                                                                ? wire_node->attributes[attr_id::packed_ranges]->children
                                                                : std::vector<AST::AstNode *>();
            const std::vector<AST::AstNode *> unpacked_ranges = wire_node->attributes.count(attr_id::unpacked_ranges)
                                                                  ? wire_node->attributes[attr_id::unpacked_ranges]->children
                                                                  : std::vector<AST::AstNode *>();
    ...
    case AST::AST_CONDZ:
        ...
        if (current_node->attributes.count(attr_id::low_high_bound)) {
    ...
    if (val.format) { // Needed to handle parameter nodes without typespecs and constants
        ...
        case vpiIntVal: {
            ...
            visit_one_to_one({vpiTypespec}, obj_h, [&](AST::AstNode *node) {
                if (node && node->attributes.count(attr_id::packed_ranges) && node->attributes[attr_id::packed_ranges]->children.size() &&
                    node->attributes[attr_id::packed_ranges]->children[0]->children.size()) {
                    size = node->attributes[attr_id::packed_ranges]->children[0]->children[0]->integer + 1;
                }
            });
            if (size == -1) {
...
static void add_or_replace_child(AST::AstNode *parent, AST::AstNode *child)
{
    if (!child->str.empty()) {
        ...
        if (it != parent->children.end()) {
            ...
            if ((*it)->attributes.count(attr_id::packed_ranges) && child->attributes.count(attr_id::packed_ranges)) {
                if ((!(*it)->attributes[attr_id::packed_ranges]->children.empty() && child->attributes[attr_id::packed_ranges]->children.empty())) {
                    child->attributes[attr_id::packed_ranges] = (*it)->attributes[attr_id::packed_ranges]->clone();
                }
            }
            if ((*it)->attributes.count(attr_id::unpacked_ranges) && child->attributes.count(attr_id::unpacked_ranges)) {
                if ((!(*it)->attributes[attr_id::unpacked_ranges]->children.empty() &&
                     child->attributes[attr_id::unpacked_ranges]->children.empty())) {
                    child->attributes[attr_id::unpacked_ranges] = (*it)->attributes[attr_id::unpacked_ranges]->clone();
                }
            }
...
void UhdmAst::process_design()
{
    ...
    for (auto &pair : shared.top_nodes) {
        if (!pair.second)
            continue;
        if (!pair.second->get_bool_attribute(attr_id::partial)) {
            if (pair.second->type == AST::AST_PACKAGE)
...
void UhdmAst::process_module()
{
    ...
    if (!is_module_instance) {
        if (shared.top_nodes.find(type) != shared.top_nodes.end()) {
            ...
            auto it = current_node->attributes.find(attr_id::partial);
            ...
            shared.current_top_node = current_node;
            current_node->attributes[attr_id::partial] = AST::AstNode::mkconst_int(1, false, 1);
            visit_one_to_many({vpiTypedef}, obj_h, [&](AST::AstNode *node) {
                if (node) {
                    move_type_to_new_typedef(current_node, node);
                }
        ...
        if (!module_node) {
            ...
            if (!module_node) {
                ...
                module_node->attributes[attr_id::partial] = AST::AstNode::mkconst_int(2, false, 1);
                cell_instance = 1;
                module_name = type;
            }
        ...
        visit_one_to_many({vpiParamAssign}, obj_h, [&](AST::AstNode *node) {
            if (node) {
                ...
                } else if ((module_node->attributes.count(attr_id::partial) && module_node->attributes[attr_id::partial]->integer == 2)) {
        ...
        if (module_node->attributes.count(attr_id::partial)) {
            AST::AstNode *attr = module_node->attributes.at(attr_id::partial);
            if (attr->type == AST::AST_CONSTANT)
                if (attr->integer == 1) {
                    delete attr;
                    module_node->attributes.erase(attr_id::partial);
                }
        }
        auto typeNode = new AST::AstNode(AST::AST_CELLTYPE);
        ...
        visit_one_to_one({vpiTypespec}, obj_h, [&](AST::AstNode *node) {
            if (node && node->type == AST::AST_STRUCT) {
                ...
                if (node->attributes.count(attr_id::packed_ranges)) {
                    for (auto r : node->attributes[attr_id::packed_ranges]->children) {
                        packed_ranges.push_back(r->clone());
                    }
                    std::reverse(packed_ranges.begin(), packed_ranges.end());
                    node->attributes.erase(attr_id::packed_ranges);
                ...
                if (node->attributes.count(attr_id::unpacked_ranges)) {
                    for (auto r : node->attributes[attr_id::unpacked_ranges]->children) {
                        unpacked_ranges.push_back(r->clone());
                    }
                    node->attributes.erase(attr_id::unpacked_ranges);
                }
...
void UhdmAst::process_param_assign()
{
    ...
    visit_one_to_one({vpiLhs}, obj_h, [&](AST::AstNode *node) {
        if (node) {
            ...
            copy_packed_unpacked_attribute(node, current_node);
            if (node->attributes.count(attr_id::is_imported)) {
                current_node->attributes[attr_id::is_imported] = node->attributes[attr_id::is_imported]->clone();
            }
    ...
    while (vpiHandle expr_h = vpi_scan(itr)) {
        ...
        if (vpi_get(vpiType, expr_h) == vpiOperation && vpi_get(vpiOpType, expr_h) == vpiInsideOp) {
            ...
            if (current_node->children.size() == 2) {
                ...
                current_node->children.clear();
                current_node->children.push_back(block);
                current_node->attributes[attr_id::low_high_bound] = AST::AstNode::mkconst_int(1, false, 1);
            }
        } else {
            UhdmAst uhdm_ast(this, shared, indent + "  ");
...
void UhdmAst::process_parameter()
{
    ...
    if (const char *imported = vpi_get_str(vpiImported, obj_h); imported != nullptr && strlen(imported) > 0) {
        current_node->attributes[attr_id::is_imported] = AST::AstNode::mkconst_int(1, true);
    }
    visit_one_to_many({vpiRange}, obj_h, [&](AST::AstNode *node) { unpacked_ranges.push_back(node); });
    vpiHandle typespec_h = vpi_handle(vpiTypespec, obj_h);
    if (typespec_h) {
        ...
        case vpiArrayTypespec: {
            ...
            visit_one_to_one({vpiElemTypespec}, typespec_h, [&](AST::AstNode *node) {
                ...
                if (node && node->attributes.count(attr_id::packed_ranges)) {
                    for (auto r : node->attributes[attr_id::packed_ranges]->children) {
                        packed_ranges.push_back(r->clone());
                    }
                }
