    ...
    if (typespec_h) {
        ...
        default: {
            ...
            report_error("Encountered unhandled typespec in process_parameter: '%s' of type '%s' at %s:%d\n", object->VpiName().c_str(),
                         UHDM::VpiTypeName(typespec_h).c_str(), object->VpiFile().c_str(), object->VpiLineNo());
            break;
        }
        }
...
AST::AstNode *UhdmAst::process_object(vpiHandle obj_handle)
{
    ...
    case vpiProgram:
    default:
        report_error("Encountered unhandled object '%s' of type '%s' at %s:%d\n", object->VpiName().c_str(), UHDM::VpiTypeName(obj_h).c_str(),
